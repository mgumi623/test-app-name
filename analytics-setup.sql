-- 分析システム用データベーススキーマ
-- このSQLファイルは以下の3つの分析用テーブルを作成します：
--   1. analytics_sessions: ユーザーセッション情報（デバイス、時間等）を追跡
--   2. analytics_events: ユーザーの行動（ページビュー、クリック等）を記録
--   3. analytics_errors: システムエラーの詳細を保存
-- 各テーブルにはインデックスとRLSポリシーが設定されます
